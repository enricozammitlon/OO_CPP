...
double matrix::determinant(matrix submatrix) const {
  ...
  for (int j{1}; j <= submatrix.columns; j += 1) {
    determinant += std::pow(-1, 1 + j) * submatrix(1, j) *
                   submatrix.determinant(submatrix.get_reduced_matrix(1, j));
  }
  return determinant;
}
  ...
  if (rows == addend.rows && columns == addend.columns) {
    for (size_t i{1}; i <= rows; i += 1) {
      for (size_t j{1}; j <= columns; j += 1) {
        addition.matrix_data[index(i, j)] =
            matrix_data[index(i, j)] + addend(i, j);
      }
    }
  } else {
...
matrix matrix::operator-(const matrix &addend) const {
  ...
  if (rows == addend.rows && columns == addend.columns) {
    for (size_t i{1}; i <= rows; i += 1) {
      for (size_t j{1}; j <= columns; j += 1) {
        subtraction.matrix_data[index(i, j)] =
            matrix_data[index(i, j)] - addend(i, j);
      }
    }
  } else {
    ...
    for (size_t i{1}; i <= rows; i += 1) {
      for (size_t j{1}; j <= multiplier.columns; j += 1) {
        ...
        for (size_t n{1}; n <= columns; n += 1) {
          product.matrix_data[index(i, j)] +=
              matrix_data[index(i, n)] * multiplier(n, j);
        }
      }
    }
  } else {
    throw "Operation not possible. Matrix A must have the same number of "
          "columns as rows in matrix B.";
  }
  return product;
}
  ...
  for (size_t i{1}; i <= rows; i += 1) {
    if (i != m) {
      ...
      for (size_t j{1}; j <= columns; j += 1) {
        if (j != n) {
          reduced_matrix
              .matrix_data[reduced_matrix.index(reduced_i, reduced_j)] =
              matrix_data[index(i, j)];
          reduced_j += 1;
        }
      }
      reduced_i += 1;
...
std::istream &operator>>(std::istream &is, matrix &output_matrix) {
  ...
  for (int j{1}; j <= matrix_to_append.columns; j += 1) {
    output_matrix.matrix_data[output_matrix.index(
        output_matrix.last_filled_row + 1, j)] = matrix_to_append(1, j);
  }
...
int main() {
  ...
  matrix a3 = a2;
  std::cout << "Matrix a3:\n" << a3 << std::endl;
  std::cout << "Contents of original matrix a2 modified for proof that copied "
               "matrix is unchanged."
            << std::endl;
  a2 = a2 + a3;
  std::cout << "Matrix a2:\n" << a2;
  ...
  matrix a4{a2};
  std::cout << "Matrix a4:\n" << a4 << std::endl;
  std::cout << "Row and Col 2,2 of original matrix a2 deleted for proof that "
               "copied matrix is unchanged."
            << std::endl;
  ...
  std::cout << "(Matrix a - Matrix b)\n" << a - b;
  std::cout << "(Matrix a * Matrix b)\n" << a * b;
  std::cout << "Determinant of Matrix a: " << a.determinant(a) << std::endl;
  return 0;
}