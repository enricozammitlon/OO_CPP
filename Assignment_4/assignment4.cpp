...
class galaxy {
private:
  // According to the hubble classification allowed values E0 to E7, S0, Sa, Sb,
  // Sc, SBa, SBb, SBc and Irr
...
void galaxy::print_data(int parenthood_depth = 0) {
  ...
  std::cout << padding << "Redshift: " << std::setprecision(2) << redshift
            << std::endl;
  std::cout << padding << "Stellar Mass Fraction: " << std::setprecision(2)
            << stellar_mass_fraction << std::endl;
  std::cout << padding << "Total Mass: " << std::setprecision(2) << total_mass
            << " M0" << std::endl;
  if (satellites.size() != 0) {
    std::cout << padding << "Satellites in this galaxy: " << std::endl;
  }
  for (auto iterator = satellites.begin(); iterator != satellites.end();
       iterator++) {
...
int main() {
  ...
  std::cout.setf(std::ios::scientific, std::ios::floatfield);
  std::cout << "\nStellar mass for satellite: " << std::setprecision(2)
            << other_galaxy.stellar_mass() << " M0\n"
            << std::endl;
